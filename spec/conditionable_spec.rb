...
RSpec.describe Configurable do
  ...
  describe '#valid_promotion?' do
    context 'when the third pawn of player one is at e7 attempts a promotion at f8' do
      ...
      context 'when it is promotable and the notation is valid' do
        it 'returns true' do
          move = ['', '', '', 'f8', '=R', nil]
          expect(game.valid_promotion?(player1.pawn[3], player1, move)).to be(true)
        end
      end
      ...
      context 'when it is promotable but the notation is invalid' do
        it 'returns true' do
          move = ['', '', '', 'f8', '', nil]
          expect(game.valid_promotion?(player1.pawn[3], player1, move)).to be(false)
        end
      end
      ...
      context 'when it is not promotable but the notation is valid' do
        it 'returns true' do
          move = ['', '', '', 'f8', '=Q', nil]
          expect(game.valid_promotion?(player1.pawn[5], player1, move)).to be(false)
        end
      end
      ...
      context 'when it is not promotable and the notation is invalid while the negate filter is on' do
        it 'returns true' do
          move = ['', '', '', '', '', nil]
          expect(game.valid_promotion?(player1.pawn[5], player1, move, negate: false)).to be(true)
        end
      end
    end
    ...
    context 'when the last seventh of player two is at h2 attempts a promotion at g1' do
      ...
      context 'when it is promotable and the notation is valid' do
        before { allow(player2).to receive(:random_destination).and_return([0, 6]) }
        it 'returns true' do
          move = ['', '', '', 'g1', '=Q', nil]
          expect(game.valid_promotion?(player2.pawn[6], player2, move)).to be(true)
        end
      end
      ...
      context 'when it is not promotable but the notation is valid' do
        ...
        it 'returns true' do
          move = ['', '', '', 'f8', '=Q', nil]
          expect(game.valid_promotion?(player2.pawn[3], player2, move)).to be(false)
        end
      end

      context 'when it is not promotable and the notation is invalid' do
        before { allow(player2).to receive(:random_destination).and_return([4, 9]) }
        move = ['', '', '', '', '', nil]
        it 'returns false' do
          expect(game.valid_promotion?(player2.pawn[5], player2, move)).to be(false)
        end
      end

      context 'when it is not promotable and the notation is invalid while the negate filter is on' do
        before { allow(player2).to receive(:random_destination).and_return([99, 18]) }
        it 'returns true' do
          move = ['', 'e4', 'x', '', '=R', nil]
          expect(game.valid_promotion?(player2.pawn[4], player2, move, negate: false)).to be(true)
        end
      end
    end
  end
  ...
  describe '#invalid_promotion' do
    context 'when the second pawn of player one is at c7 attempts a promotion at d8' do
      ...
      context 'when the move elements are legitimate and match a promotion' do
        it 'returns nil' do
          move = ['', '', '', 'd8', '=N', nil]
          expect(game.invalid_promotion(player1.pawn[1], move, player1)).to be_nil
        end
      end
      ...
      context 'when the move elements are legitimate but they do not match a promotion' do
        it 'returns nil' do
          move = ['N', '', '', 'h3', '', nil]
          expect(game.invalid_promotion(player1.knight[1], move, player1)).to be_nil
        end
      end

      context 'when the move elements are legitimate and match a promotion' do
        let(:move) { ['', '', '', 'd8', '', nil] }
        let(:action) { game.invalid_promotion(player1.pawn[1], move, player1) }
      ...
      context 'when the move elements are legitimate but they do not match a promotion' do
        let(:move) { ['', '', '', 'h8', '=N', nil] }
        let(:action) { game.invalid_promotion(player1.pawn[7], move, player1) }
        it 'returns false and prints the error message' do
    ...
    context 'when the last pawn of player two is at h2   attempts a promotion at g1' do
      ...
      context 'when the move elements are legitimate and match a promotion' do
        ...
        it 'returns nil' do
          move = ['', '', '', 'g1', '=Q', nil]
          expect(game.invalid_promotion(player2.pawn[7], move, player2)).to be_nil
        end
      end

      context 'when the move elements are legitimate but they do not match a promotion' do
        before { allow(player2).to receive(:random_destination).and_return([4, 4]) }

        it 'returns nil' do
          move = ['', '', '', 'e5', '', nil]
          expect(game.invalid_promotion(player2.pawn[4], move, player2)).to be_nil
        end
      end
    end
  end
  ...
  describe '#validate_promotion' do
    let(:human_pawn) { player1.pawn }
    let(:npc_pawn) { player2.pawn }
    ...
    context 'when the sixth pawn of player two at g2 moves to f1 ' do
      before do
        ...
        allow(player2).to receive(:random_destination).and_return([0, 5])
      end
      ...
      context 'when the promotion is valid' do
        ...
        it 'changes the position of player one\'s bishop to nil (captured)' do
          expect { action }.to change { player1.bishop[1].current_position }.from([0, 5]).to(nil)
        end
      end
    end
  end
  ...
  describe '#invalid_moves' do
    context 'when the active piece is the king of the first player' do
      it 'returns nil' do
        expect(game.invalid_moves(player1, player1.king)).to be_nil
      end
    end

    context 'when the active pieces are all pawns of the first player' do
      let(:action) { game.invalid_moves(player1, player1.pawn) }
    ...
    context 'when the pawn of player one at d5 makes an en passant move to e7' do
      before do
      ...
      let(:action) { game.invalid_moves(player1, player1.pawn, [5, 4]) }

      it 'prints the error message and returns false' do
        expect(game.en_passant?(player1, [5, 4])).to be(true)
    ...
    context 'when the active pieces empty for the first player' do
      let(:action) { game.invalid_moves(player1, []) }
      it 'prints the error message and returns false' do
    ...
    context 'when the active piece is the queen of the second player' do
      it 'returns nil' do
        expect(game.invalid_moves(player2, player2.queen)).to be_nil
      end
    end

    context 'when the active pieces are both knights of the second player' do
      let(:action) { game.invalid_moves(player2, player2.knight) }
    ...
    context 'when the pawn of player two at g4 makes an en passant move to f3' do
      before do
      ...
      let(:action) { game.invalid_moves(player2, player2.pawn, [2, 5]) }

      it 'prints the error message and returns false' do
        expect(game.en_passant?(player2, [2, 5])).to be(true)
        msg = "\nThis is an en passant move. The notation should start with the file of the pawn performing the move.\n"
        expect { action }.to output(include(msg)).to_stdout
        expect(action).to be(false)
      end
    ...
    context 'when the active pieces empty for the second player' do
      let(:action) { game.invalid_moves(player2, []) }
  ...
  describe '#active_piece_conditions' do
    context 'when the first pawn of player two is selected' do
      let(:pawn) { player2.pawn[0] }
      before { allow(player2).to receive(:random_destination).and_return([5, 0]) }

      it 'returns false for captured pieces' do
        pawn.current_position = nil
        expect(game.active_piece_conditions(pawn, player2, [5, 0], nil, nil)).to be(false)
      end

      it 'returns true for valid moves without origin' do
        expect(game.active_piece_conditions(pawn, player1, [5, 0], nil, nil)).to be(true)
      end

      it 'filters by origin when provided' do
        expect(game.active_piece_conditions(pawn, player1, [5, 0], [0], [1])).to be(true)
        expect(game.active_piece_conditions(pawn, player1, [5, 0], [6], [0])).to be(true)
        expect(game.active_piece_conditions(pawn, player1, [5, 0], [6, 0], [0, 1])).to be(true)
        expect(game.active_piece_conditions(pawn, player1, [5, 0], [2], [0])).to be(false)
      end
    end

    context 'when the rook on the queen side moves to e1' do
      let(:rook) { player1.rook[0] }
      before do
        0.upto(7) do |idx|
      ...
      it 'returns false for captured pieces' do
        ...
        expect(game.active_piece_conditions(rook, player1, [0, 4], nil, nil)).to be(false)
      end

      it 'returns true for valid moves without origin' do
        expect(game.active_piece_conditions(rook, player1, [0, 4], nil, nil)).to be(true)
      end

      it 'filters by origin when provided' do
        expect(game.active_piece_conditions(rook, player1, [0, 4], [0], [1])).to be(true)
        expect(game.active_piece_conditions(rook, player1, [0, 4], [0], [0])).to be(true)
        expect(game.active_piece_conditions(rook, player1, [0, 4], [0, 0], [0, 1])).to be(true)
        expect(game.active_piece_conditions(rook, player1, [0, 4], [2], [0])).to be(false)
      end
    end
  end

  describe '#correct_path?' do
    let(:idx) { [0, 1] }

    context 'when not en passant move' do
      let(:pawn) { player1.pawn[5] } # a2 pawn
      let(:destination) { [3, 5] }    # a3

      it 'returns true when origin and idx are nil' do
        expect(game.correct_path?(pawn, player1, destination, nil, nil)).to be true
      end

      it 'returns true when origin is nil' do
        expect(game.correct_path?(pawn, player1, destination, nil, idx)).to be true
      end

      it 'returns true when origin doesnt match piece position' do
        expect(game.correct_path?(pawn, player1, destination, nil, [0])).to be(true)
      end

      it 'returns false when origin doesnt match piece position' do
        expect(game.correct_path?(pawn, player1, destination, [1, 5], idx)).to be(true)
      end

      it 'returns false when origin doesnt match piece position' do
        expect(game.correct_path?(pawn, player1, destination, [5], [1])).to be(true)
      end

      it 'returns false when origin doesnt match piece position' do
        expect(game.correct_path?(pawn, player1, destination, [1], [1])).to be(false)
      end
    end

    context 'when en passant move' do
      before do
      ...
      let(:pawn) { player2.pawn[3] }
      let(:destination) { [2, 2] }

      it 'returns nil when origin and idx are nil' do
        expect(game.correct_path?(pawn, player2, destination, nil, nil)).to be_nil
      end

      it 'returns false when idx is nil' do
        expect(game.correct_path?(pawn, player2, destination, nil, idx)).to be_nil
      end

      it 'returns false when idx is nil' do
        expect(game.correct_path?(pawn, player2, destination, [3], nil)).to be(false)
      end

      it 'returns true when origin and idx are correct' do
        expect(game.correct_path?(pawn, player2, destination, [3], [0])).to be(true)
      end

      it 'returns true when origin and idx are correct' do
        expect(game.correct_path?(pawn, player2, destination, [3, 3], [0, 1])).to be(true)
      end

      it 'returns true when origin and idx are correct' do
        expect(game.correct_path?(pawn, player2, destination, [3], [1])).to be(true)
      end
    end

    context 'when the knight of player one moves' do
      let(:knight) { player1.knight[1] }
      let(:destination) { [2, 7] }

      it 'returns true when origin and idx are nil' do
        expect(game.correct_path?(knight, player1, destination, nil, nil)).to be(true)
      end

      it 'returns false when idx is nil' do
        expect(game.correct_path?(knight, player1, destination, [6], nil)).to be(false)
      end

      it 'returns true when origin is nil' do
        expect(game.correct_path?(knight, player1, destination, nil, idx)).to be(true)
      end

      it 'returns true when origin and idx are correct' do
        expect(game.correct_path?(knight, player1, destination, [0, 6], [0, 1])).to be(true)
      end

      it 'returns true when origin and idx do not match' do
        expect(game.correct_path?(knight, player1, destination, [0], [1])).to be(false)
      end

      it 'returns true when origin and idx do not match' do
        expect(game.correct_path?(knight, player1, [7, 6], [6], [0])).to be(false)
      end
    end

    context 'when both rook of player two can move to the same position' do
      before do
        0.upto(7) do |idx|
          unless [0, 4, 7].include?(idx)
            game.board.layout[7][idx].current_position = nil
      ...
      let(:rook) { player2.rook[1] }
      let(:destination) { [7, 4] }


      it 'returns nil when origin and idx are nil' do
        expect(game.correct_path?(rook, player2, destination, nil, nil)).to be(true)
      end

      it 'returns false when origin is nil' do
        expect(game.correct_path?(rook, player2, destination, nil, idx)).to be(true)
      end

      it 'returns false when idx is nil' do
        expect(game.correct_path?(rook, player2, destination, [7], nil)).to be(false)
      end

      it 'returns true when origin and idx are correct' do
        expect(game.correct_path?(rook, player2, destination, [7], [0])).to be(true)
      end

      it 'returns true when origin and idx are correct' do
        expect(game.correct_path?(rook, player2, destination, [7], [1])).to be(true)
      end

      it 'returns true when origin and idx are correct' do
        expect(game.correct_path?(rook, player2, destination, [7, 7], [0, 1])).to be(true)
      end
    end
  end
  ...
  describe '#active_pieces' do
    context 'when the destination is set at f5 and the active pieces are the pawns of player two' do
      ...
      it 'returns the pawn at f5 of player two' do
        action = game.active_pieces(player2.pawn, player2, [4, 5])
        expect(action).to eq([player2.pawn[5]])
      end
    end
    ...
    context 'when the destination is set at f5 and the active pieces are the pawns of player one' do
      ...
      it 'returns the pawn at f6 of player one' do
        action = game.active_pieces(player1.pawn, player1, [6, 4], [5], [1])
        expect(action).to eq([player1.pawn[5]])
      end
    end
    ...
    context 'when the destination is set at a5 and the active pieces are the rooks of player two' do
      before do
        ...
        allow(player2).to receive(:random_destination).and_return([4, 0])
      end

      it 'returns the rook at a6 of player two' do
        action = game.active_pieces(player2.rook, player2, [4, 0], [3], [0])
        expect(action).to eq([player2.rook[1]])
      end
    end
    ...
    context 'when the destination is set d5 and the active pieces are the knights of player one' do
      ...
      it 'returns the knight at f4 of player one' do
        action = game.active_pieces(player1.knight, player2, [4, 3], [3, 5], [0, 1])
        expect(action).to eq([player1.knight[1]])
      end
    end
    ...
    context 'when the destination is set at d4 and the active pieces are the knights of player two' do
      before do
        ...
        allow(player2).to receive(:random_destination).and_return([3, 3])
      end

      it 'returns both knights of player two' do
        action = game.active_pieces(player2.knight, player2, [3, 3])
        expect(action).to eq(player2.knight)
      end
    end

    context 'when the destination is set at d3 and the active piece is the queen of player one' do
      it 'returns empty array' do
        action = game.active_pieces(player1.queen, player1, [2, 3])
    ...
    context 'when the active pieces are empty' do
      it 'returns empty array' do
        action = game.active_pieces([], player1, [7, 7])
        expect(action).to be_empty
      end
    end
  end
  ...
  describe '#winner?' do
    context 'when the king of player two is captured' do
      before { player2.king[0].current_position = nil }
    ...
    context 'when there is no win condition' do
      it 'returns nil for player one and does not print the winner message' do
        ...
        msg = "\nPlayer 1 is the winner!"
        expect { game.winner?(player1) }.not_to output(include(msg)).to_stdout
      end

      it 'returns nil for player two and does not print the winner message' do
        expect(game.winner?(player2)).to be_nil
        msg = "\nPlayer 2 is the winner!"
        expect { game.winner?(player2) }.not_to output(include(msg)).to_stdout
      end
    end
  end
  ...
  describe '#win_condition' do
    context 'when the current position of the king of the second player is set as nil' do
      ...
      it 'returns false for player two and true for player one' do
        expect(game.win_condition(player2)).to be(false)
  ...
  describe '#king_captured?' do
    context 'when the king of the second player is examined at the beginning' do
      it 'returns false' do
        expect(game.king_captured?(player1)).to be(false)
      end
    end

    context 'when the king of the first player is examined at the beginning' do
      it 'returns false' do
        expect(game.king_captured?(player2)).to be(false)
      end
    end

    context 'when the current position of the king of the first player is set as nil' do
      before { player1.king[0].current_position = nil }
      it 'returns true' do
